...
@Component({
  ...
  styleUrls: ['../tasks.component.css', './test-tasks.component.css']
})
export class TestTasksComponent {
  @Input() todoTasks = [];
  @Input() progressTasks = [];
  @Input() testTasks = [];
  @Input() doneTasks = [];
  ...
  handleEnter(event: Event) {
    ...
    if (this.isCreating) {
      if (input != '' && input != null) {
        this.testTasks.unshift(input);
      }
    } else if (this.isEditing) {
      this.testTasks[this.editedTaskIndex] = input;
    }
  ...
  handleMove(taskIndex:number, statusIndex:number) {
    this.isMoving = false;
    if (statusIndex === 0) {
      this.todoTasks.unshift(this.testTasks[taskIndex]);
    } else if (statusIndex === 1) {
      this.progressTasks.unshift(this.testTasks[taskIndex]);
    } else {
      this.doneTasks.unshift(this.testTasks[taskIndex]);
    }

    this.testTasks.splice(taskIndex, 1);
  }
}
